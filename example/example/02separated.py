...
from goconvert.output import SeparatedOutput
...
def run(src_file, package_name, src_package, dst):
    reader = Reader()
    ...
    for file in world.modules[src_package].files.values():
        new_file = output.new_file(file.name)
...
def main():
    ...
    return run(args.src, args.package, args.src_package, args.dst)
